...
function createComponent(vm,tag,data,key,children,Ctor){
    if(isObject(Ctor)){
        Ctor = vm.$options._base.extend(Ctor); // 到这来已经都是构造函数了
    }
    // 给组件增加生命周期
    data.hook = {
        init(vnode){ // 初始化的钩子
    ...
    // 组件的虚拟节点拥有hook  当前组件的componentOptions 中存放了组件的构造函数
    return vnode(vm, `vue-component-${Ctor.cid}-${tag}`,data,key,children, undefined, {Ctor})
}
